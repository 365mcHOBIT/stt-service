...
export async function GET(request, { params }) {
  try {
    const data = await ExecuteQuery(`SELECT TOP 1 * FROM CORE_ADMINS A, CORE_ADMIN_POSITION P
      WHERE A._id = ${params.id} AND A.POSITION_id = P._id`);
    ...
    if(positionID == 11) {
      ...
      data1 = await ExecuteQuery(`SELECT COUNT(*) AS NOT_READ FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND STATUS_NUMBER = 0`);
      data2 = await ExecuteQuery(`SELECT COUNT(*) AS WAITING_EDIT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND STATUS_NUMBER = 1`);
      data3 = await ExecuteQuery(`SELECT COUNT(*) AS COMPLETE_EDIT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND STATUS_NUMBER = 2`);
      data4 = await ExecuteQuery(`SELECT COUNT(*) AS CNT, SUM(DATEDIFF(ss, START_TIME, END_TIME)) AS AMOUNT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND START_TIME = GETDATE()`);
      data5 = await ExecuteQuery(`SELECT COUNT(*) AS CNT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND YEAR(START_TIME) = YEAR(GETDATE()) AND MONTH(START_TIME) = MONTH(GETDATE())`);
      data6 = await ExecuteQuery(`SELECT COUNT(*) AS CNT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND YEAR(START_TIME) = YEAR(GETDATE())`);
      data7 = await ExecuteQuery(`SELECT COUNT(*) AS CNT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND YEAR(START_TIME) = YEAR(GETDATE())-1 AND MONTH(START_TIME) = MONTH(GETDATE())`);
      data8 = await ExecuteQuery(`SELECT COUNT(*) AS CNT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id} AND YEAR(START_TIME) = YEAR(GETDATE())-1`);
      data9 = await ExecuteQuery(`SELECT COUNT(*) AS CNT, SUM(DATEDIFF(ss, START_TIME, END_TIME)) AS AMOUNT FROM CORE_CONSULTS WHERE ADMIN_id = ${params.id}`);
    }
